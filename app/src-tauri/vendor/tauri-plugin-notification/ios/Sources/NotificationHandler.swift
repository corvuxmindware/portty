...
public class NotificationHandler: NSObject, NotificationHandlerProtocol {
  ...
  func toActiveNotification(_ request: UNNotificationRequest) -> ActiveNotification {
    // PORTTY PATCH (the only change in this vendored copy): upstream is
    // `notificationsMap[request.identifier]!`. That map is in-memory and only
    // holds entries THIS process posted, so tapping a banner after iOS reaped
    // the app looks up an absent key and force-unwraps nil - crashing on
    // launch. For Portty that is the main path, not an edge case: you
    // background the app, an approval arrives, iOS suspends and then reaps us,
    // and the tap is how you reach the blocked agent. Safe optional lookup with
    // the defaults the encoder already tolerates.
    //
    // Found via Corvux, which carries the same fix and reported it still broken
    // upstream as of 2026-06-14. Drop this vendor when upstream makes the
    // lookup safe.
